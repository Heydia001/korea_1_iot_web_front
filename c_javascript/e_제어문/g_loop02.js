...
while (조건식) {
== 반복한 코드 블록 ==
    >> 표현식의 결과가 조건식의 falsw(거짓)에 도달 할 수 있도록
    표현식을 변경하는 실행문을 반드시! 포함 

    >> 표현식을 변경하는 실행문이 없을 경우 >> 무한 반복("무한 루프 실행")
}
*/ 

console.log('== for 반복문 ==');
//a를 출력, 0 부터 4까지 1씩 증가하는 수
// for (let a = 0; a < 5; a++){
//   console.log(num);
// }
...
while (b < 5){        // 조건식
  ...
  // 증감식 : 코드 블럭 내에 존재
  // > while문은 조건에 도달하지 않으면 코드블럭의 완전한 반복 후 다시 조건을 검토
  b++;
...
// >> 최소한 한 번의 코드블럭 실행을 보장한다.

//& do-while 문의 기본 구조(형태)
...
let c = 0;
do {
  // 실행 블록내에 초기화 식 작성의 경우
  // : 반복할 때 마다 초기화 되어 무한 루프와 동일
  console.log(c);
  c += 1;
} while (c < 5);

console.log('===반복문 / 조건문 활용의 예제===');
