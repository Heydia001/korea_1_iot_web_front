...
//!  2. 이벤트의 기본 행동 방지
// form 태그의 경우 submit 버튼 실핼 시 내부의 데이터가 서버에 전송되고 페이지가 리로드된다.
// >> 기본행동을 방지하는 메서드: preventDefault()
...
childButton.addEventListener('click', function(e) {
  ...
  //이벤트 버블링은 자식에서 부모로 진행된다.
  // :이벤트 객체.stopPropagation() 메서드를 사용해서 전파를 중지
  e.stopPropagation(); //자식요소에서 지정한다.
})