...
// EX) API 호출, 이벤트 리스너 등록(거의 사용 X), 수동 DOM 조작 등
...
export default function UseEffect01() {
  ...
  const [name, setName] = useState<string>('김다혜');
  ...
  //? 두 번째 인자(의존성 배열, deps)
  // : 해당 배열의 값이 변경될 때 마다 부수효과가 다시 실행됨
  // >> 빈 배열: 컴포넌트가 마운트 될 때만 한 번만 실행
  // +) 의존성 배열에 값이 있더라고 마운트 시에는 반그시 실핼

  useEffect(() => {
    // 부수 효과를 작성
    console.log(`Count: ${count}`);
    // 의존 배열에 작성된 데이터의 상태 변화가 일너난 경우
    // >> 효과를 재실행
  }, [count, name]);
  ...
  useEffect(() => {
    console.log('컴포넌트에 렌더링');
    // 의존성 배열이 없는 경우: 모든 랜더링마다 실행
  }, [])
  ...
  return (
    <div>
      ...
      <br />
      <button onClick={() => setName(name === '김다혜' ? '김다혜' : '김도결')}>이름 변경</button>
    </div>
  )
}